...
path_images = './HeadPoseImageDatabase'
images = glob.iglob(path_images+"/**/*.jpg")
...
for path_image in images:
    ...
    time.sleep(0.10)
    if len(faces) == 0:
        cv.imwrite(output_dir+name_image+"_not_work."+name_extension, image)
        continue
    for (x,y,w,h) in faces:
        ...
        import os
        output_dir = path_fil+"/output_faced/"
        if not os.path.exists(output_dir):
            os.mkdir(output_dir)

     
        cv.imwrite(output_dir+name_image+"_"+str(num_face)+"."+name_extension, image)
        num_face+=1
